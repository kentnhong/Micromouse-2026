...
constexpr Gains kVelocityPid{
    0.002f, 0.0f, 0.0f};  // Output is normalized motor drive per ticks/s error

constexpr float kTargetTicksPerSec = 400.0f;
static constexpr uint32_t kEncoderSample =
    1'000;  // 1KHZ sampling rate for encoder readings
...
const Stmf4::StGpioParams left_in1_params{
    4, GPIOA, motor_input_settings};  // PA4 channel A
const Stmf4::StGpioParams left_in2_params{
    5, GPIOA, motor_input_settings};  // PA5 channel B
...
static PID left_pid(config);
static PID right_pid(config);
static float left_target_ticks_per_sec = kTargetTicksPerSec;
static float right_target_ticks_per_sec = kTargetTicksPerSec;
...
{
    return std::tie(left_pid, right_pid, left_target_ticks_per_sec,
                    right_target_ticks_per_sec);
}
}  // namespace MM
